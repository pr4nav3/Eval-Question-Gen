...
This prompt intentionally allows easy rows when they are useful recall evals.
It should be paired with `--min-difficulty easy` or
`--judge-min-difficulty easy`; otherwise the deterministic gate will still
reject easy rows.
...
unfair traps, boilerplate rows, and duplicate same-run ideas.
